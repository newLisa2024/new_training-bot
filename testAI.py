...
def get_feedback(question, user_response):
    prompt = f"""Действуй как опытный HR-программист, специалист по python.
    Ты получил вопрос: "{question}"
    и ответ соискателя: "{user_response}".
    Ты должен оценить ответ и дать обратную связь. Есть два варианта начала ответа : 'не правильный ответ' и 'правильный ответ'. Правильный ответ ты пишешь если ответ правльный, не правильный ответ ты пишешь если ответ неправильный или неполный. Если ответ не правильный, потом напиши правильный ответ. 
